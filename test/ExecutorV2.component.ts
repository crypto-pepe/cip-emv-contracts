...
import { deployExecutorV2 } from '../steps/executorV2.fixtures';
...
describe('ExecutorV2 component', function () {
    describe('init tests', function () {
        it('should throw when admin has zero address', async () => {
            const { executor, other } = await loadFixture(deployExecutorV2);
            await expect(
        ...
        it('should throw when signer has zero address', async () => {
            const { executor, admin } = await loadFixture(deployExecutorV2);
            await expect(
        ...
        it('simple positive', async () => {
            const { executor, admin, other } = await loadFixture(deployExecutorV2);
            await executor.init(
                admin.address,
    ...
    describe('updateSigner tests', function () {
        ...
        it('simple positive', async () => {
            const { executor, admin, other, third } = await loadFixture(deployExecutorV2);
            await executor.init(
                admin.address,
                EXTERNAL_SOURCE_CHAIN_ID,
                other.address
    ...
    describe('execute tests', function () {
        ...
        it('simple positive', async () => {
            const { executor, caller, admin, other } = await loadFixture(deployExecutorV2);
            await executor.init(
                admin.address,
                EXTERNAL_SOURCE_CHAIN_ID,
                other.address
        ...
        it('reverted without error (when calldata is wrong)', async () => {
            const { executor, caller, admin, other, third } = await loadFixture(deployExecutorV2);
            await executor.init(
                admin.address,
                EXTERNAL_SOURCE_CHAIN_ID,
                other.address
